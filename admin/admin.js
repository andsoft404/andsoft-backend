...
(function () {
  ...
  function api(action, data) {
    if (typeof BACKEND === 'undefined') {
      return Promise.reject(new Error('Backend холбогдоогүй. InfinityFree дээр тест хийнэ үү.'));
    }
  ...
  function saveContent(type, data) {
    DB[type] = data;
    return api(type + '.save', data).catch(function(e) { console.warn('Save ' + type + ':', e.message); });
  }

  function saveArray(type, data) {
    DB[type] = data;
    return api(type + '.replaceAll', { items: data }).catch(function(e) { console.warn('Save ' + type + ':', e.message); });
  }

  function savePricing(data) {
    DB.pricing = data;
    return api('pricing.replaceAll', { categories: data }).catch(function(e) { console.warn('Save pricing:', e.message); });
  }
  ...
  function loadProfile() {
    ...
    if (avatarEl) avatarEl.src = p.avatar ? p.avatar : '../assets/images/Logo1.png';
  }
  ...
  function initPanel() {
    ...
    $('editProfileBtn').addEventListener('click', function () {
      ...
      avatar.src = p.avatar ? p.avatar : '../assets/images/Logo1.png';
